...
% Add libraries
addpath(genpath([pwd '/libraries/MIRtoolbox1.7.2']));
addpath(genpath([pwd '/libraries/mPraat-master']));

% Gather filepaths
clip_loc = ['C:\Users\hleerubin\Desktop\last_file'];
subclip_loc = [pwd '/ids_subclips']; % reconcatenated with concatenate.py
tg_loc = [pwd '/textgrids'];
...
for i = 1:length(subclip_files)
    try
        ...
        [attackslope, events] = mirattackslope([clip_loc '/' clip_files(i).name]);
...
for i = 1:length(subclip_files)
    ...
    if ~isempty(frames)
        ...
        % Compute change in slope per event
        reconcatenated_attack(i).mir_attack_dist = sum(abs(diff(frames)));
    end
end

% Save summary in .csv
attack_summary = reconcatenated_attack;
attack_summary = rmfield(attack_summary, {'events', 'attackslope'});
...
for i = 1:length(subclip_files)
    ...
    quantiles = quantile(frames, [0 0.25 0.5 0.75 1]);
    roughness_rolloff_inharm(i).mir_roughness_min = quantiles(1);
    roughness_rolloff_inharm(i).mir_roughness_first_quart = quantiles(2);
    roughness_rolloff_inharm(i).mir_roughness_median = quantiles(3);
    roughness_rolloff_inharm(i).mir_roughness_third_quart = quantiles(4);
    roughness_rolloff_inharm(i).mir_roughness_max = quantiles(5);
    roughness_rolloff_inharm(i).mir_roughness_iqr = iqr(frames);
    
    % Compute change in roughness per second
    roughness_rolloff_inharm(i).mir_roughness_dist = velocity(frames, frames_pos);
end
